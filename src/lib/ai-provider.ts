...
2. **任務拆解與指派**：在你的回覆文字中，向使用者說明你如何拆解任務，並指派工作給所有的四位專家。你必須說明 Maya、Leon、Iris 與 Jack 分別負責哪些內容。
3. **子提示詞輸出格式 (關鍵)**：
...
- 核心主張：很多問題不是你不夠努力，而是你還沒看見真正影響結果的關鍵因素。`;
  ...
  try {
    ...
    console.error("OpenAI call for Erick COO failed, falling back to mock:", error);
    return parseCOOOutput(await callMockCOO(history[history.length - 1]?.content || "", brandName));
  }
...
2. **長篇且結構完整**：Maya 產出的文章/社群文案必須是一篇「內容豐富、長篇且結構完整」的文章，包含引人入勝的導言、條列式核心論點、具體案例與步驟分析，以及文末行動呼籲與標籤。嚴禁簡短或敷衍的摘要。
...
請確保 JSON 格式完全正確，沒有任何額外的解釋文字。`;
  ...
  try {
    ...
    console.error("Parallel AI calls failed, falling back to mock:", error);
    return parseCOOOutput(await callMockCOO(history[history.length - 1]?.content || "", brandName));
  }
...
async function callMockCOO(userInput: string, brandName: string): Promise<string> {
  ...
  if (isI8) {
    socialCopy = `# 為什麼老闆越忙，公司越長不大？ (Maya 產出)
...
#Initial8 #決策校準 #企業診斷 #中小企業主 #團隊節奏`;
  ...
  - 專欄文章 (決策/團隊/搬遷與轉型)`;
    ...
    socialCopy = `# 你不是想太多，你只是對感受比較敏銳 (Maya 產出)
...
#平衡空間 #生命數字 #自我探索 #高敏感 #人生節奏`;
  ...
  - 聯絡諮詢`;
    ...
    socialCopy = `# 你不是不夠努力，而是你的狀態需要被重新支持與調和 (Maya 產出)
...
#艾伯林量子調頻 #個人狀態調和 #量子調和 #內在消耗 #生命狀態校準`;
  ...
  - 聯絡諮詢`;
    ...
    // 個人品牌
    socialCopy = `# 很多問題不是你不夠努力，而是你還沒看見關鍵因素 (Maya 產出)
...
請確保 JSON 語法完全正確。`;
  ...
  try {
    ...
    console.error("API call for AEO generation failed, falling back to mock:", error);
  }
